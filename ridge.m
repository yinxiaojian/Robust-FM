function [ model, metric ] = ridge( training, validation, pars )
...
%                 y_predict = w0 + W(nz_idx)*X(nz_idx)' + factor_part;
                ...
                if strcmp(task, 'binary-classification')
                    ...
                    w0_ = learning_rate / (idx + t0) * (y_predict - y);
                    w0 = w0 - w0_;
                    W_ = learning_rate / (idx + t0) * ((y_predict - y)*X(nz_idx) + 2 * reg * W(nz_idx));
                ...
                if strcmp(task, 'multi-classification')
                    ...
                    for u=1:class_num
                        if err(u) > 0
                            w0_ = learning_rate / (idx + t0) * (y_predict(u) - y(u));
                            w0(u) = w0(u) - w0_;
                            W_ = learning_rate / (idx + t0) * ((y_predict(u) - y(u))*X(nz_idx) + 2 * reg * W(u,nz_idx));
